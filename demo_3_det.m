...
% prepare image info
image = imread('../data/custom_masks/resized.0000000.png');
...
if strfind(model_select, 'det')
    ...
    detection_visualization(image+r, boxes, net, config);
        ...
        % prepare segmentation data - finds number of unique classes
        seg_mask_ori = imread('../data/custom_masks/resized_gray.0000000.png'); 
        seg_mask_ori(seg_mask_ori == 255) = 0; % ignore white space
        gt_idx = unique(seg_mask_ori);
        gt_idx(gt_idx == 0) = []; % ignore class background
        gt_idx = length(gt_idx);

%         gt_idx = 14; % number of unique classes in SYNTHIA-SF dataset

        [~, target_idx_candidate_shuffle] = generate_mapping(gt_idx);        
%         load(sprintf('../data/%s.mat', shape)); % load pre-defined mask
        load(sprintf('../data/custom_masks/mask_0000000.png.mat')); % load pre-defined mask
